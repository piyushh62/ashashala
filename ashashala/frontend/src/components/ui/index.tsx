  ...
  return (
    <div
      className={`bg-white rounded-2xl border border-slate-200/70 shadow-card ${
        hover ? "transition hover:shadow-soft hover:border-brand-200" : ""
      } ${className}`}
    >
      {children}
    </div>
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-slate-100">
      <div className="flex items-center gap-3 min-w-0">
        {icon && (
          <div className="w-9 h-9 rounded-xl bg-brand-50 text-brand-600 grid place-items-center text-lg shrink-0">
            {icon}
          </div>
        )}
...
const fieldCls =
  "w-full px-3.5 py-2.5 rounded-xl border border-slate-300 bg-white text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-200 focus:border-brand-400 transition";
...
export function Spinner({ label }: { label?: string }) {
  return (
    <div className="flex items-center gap-2 text-slate-500 text-sm">
      <span className="inline-block w-4 h-4 border-2 border-slate-200 border-t-brand-600 rounded-full animate-spin" />
      {label}
    </div>
  );
}
...
export function EmptyState({ title, hint, icon = "✨" }: { title: string; hint?: string; icon?: ReactNode }) {
  return (
    <div className="text-center py-14 px-6">
      <div className="mx-auto w-12 h-12 rounded-2xl bg-slate-100 grid place-items-center text-2xl mb-3">{icon}</div>
    ...
    <Card className="p-5 animate-slide-up">
      <div className="flex items-start justify-between">
        <div className="min-w-0">
          ...
          <div className="text-3xl font-bold text-slate-800 mt-1 tabular-nums truncate">{value}</div>
